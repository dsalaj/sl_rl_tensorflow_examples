...
def iterate_minibatches(action_one_hots, states, rewards, batchsize, shuffle=False):
    ...
    for start_idx in range(0, len(action_one_hots) - batchsize + 1, batchsize):
      ...
      actions_exc = [action_one_hots[i] for i in excerpt]
      states_exc = [states[i] for i in excerpt]
      rewards_exc = [rewards[i] for i in excerpt]
      yield actions_exc, states_exc, rewards_exc
