...
def simulate(lmbda):
    ...
    def update_Q_table(Q_table, state, action, reward, new_state, new_action, is_done):
        ...
        if Q_table[new_state, max_action] == Q_table[new_state, new_action]:
            max_action = new_action

        if is_done:
            d = (reward - Q_table[state, action])
        else:
            d = (reward + gamma * Q_table[new_state, max_action] - Q_table[state, action])
        ...
        for s in range(Q_table.shape[0]):
            for a in range(Q_table.shape[1]):
                ...
                if max_action == new_action:
                    eligibility_traces[s, a] *= gamma * lmbda
                else:
                    eligibility_traces[s, a] = 0
