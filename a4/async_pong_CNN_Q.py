...
class QNetwork():
    def __init__(self, s_size, a_size, scope, trainer):
        with tf.variable_scope(scope):
            ...
            # Q functions
            self.target_Q = tf.placeholder(dtype=tf.float32, shape=(None, n_actions), name='symbolic_state2')
            ...
            gamma_rew = tf.reshape(gamma * tf.reduce_max(self.target_Q, reduction_indices=1) * (1 - self.is_done_holder), (-1, 1, 1))
...
def policy(state, epsilon, target_Q_net):
    ...
    Q_values = sess.run(target_Q_net.Q,
                        feed_dict={target_Q_net.state_holder: state.reshape(1, img_size, img_size, input_ch_num)})
...
class Worker():
    ...
    def train(self, global_Q, rollout, sess, gamma, bootstrap_value, apply_gradients=False):
        ...
        target_feed_dict = {
            self.local_target_Q.state_holder: next_observations.reshape((-1, img_size, img_size, input_ch_num)),
        }
        target_next_Q = sess.run(self.local_target_Q.Q, feed_dict=target_feed_dict)
        feed_dict = {
                     self.local_Q.target_Q: target_next_Q.reshape(-1, n_actions),
                     self.local_Q.r_holder: rewards.reshape(-1, 1, 1),
                     self.local_Q.state_holder: observations.reshape((-1, img_size, img_size, input_ch_num)),
        ...
        with sess.as_default(), sess.graph.as_default():
            while not coord.should_stop():
                ...
                while not done:
                    ...
                    action_idx = policy(pro_observation, self.epsilon, self.local_target_Q)
                    ...
                    episode_buffer.append([pro_observation, pro_new_observation, action_arr, reward, done])
                    ...
                    if total_steps % async_update_steps == 0:
                        ...
                        episode_buffer = []
                    elif reward != 0 or (len(episode_buffer) > 0 and done):
                    ...
                    target_feed_dict = {
                        self.local_target_Q.state_holder: pro_new_observation.reshape((-1, img_size, img_size, input_ch_num)),
                    }
                    target_next_Q = sess.run(self.local_target_Q.Q, feed_dict=target_feed_dict)
                    err = sess.run(self.local_Q.loss, feed_dict={
                        self.local_Q.target_Q: target_next_Q.reshape(-1, n_actions),
                        self.local_Q.state_holder: pro_observation.reshape((-1, img_size, img_size, input_ch_num)),
                        self.local_Q.action_holder: action_arr.reshape(-1, n_actions, 1),
                        self.local_Q.is_done_holder: np.array(done).reshape(-1,),
                        self.local_Q.r_holder: np.array(reward).reshape(-1, 1, 1)})
                ...
                if episode_count % 20 == 0:
                    if not os.path.exists(self.name + 'CNNQ' + '/'):
                        os.makedirs(self.name + 'CNNQ' + '/')
                    saver.save(sess, self.name + 'CNNQ' + '/network' + '-parl', global_step=episode_count)
...
with tf.device("/cpu:0"):
    ...
    trainer = tf.train.RMSPropOptimizer(learning_rate, decay=0.99, momentum=0.0, epsilon=1e-6)
