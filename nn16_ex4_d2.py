...
for e_i in range(n_epochs):
  ...
  if es_acc >= best_es_validation:
    ...
    best_train_error = train_accs[-1]
  else:
    break

print("Early stopping at epoch number =", best_epoch_num)
...
while True:
  ...
  train_accs.append(1 - sess.run(accuracy, feed_dict={x: X, y_: C_onehot}))
  test_mcs.append(1 - sess.run(accuracy, feed_dict={x: X_tst, y_: C_tst_onehot}))
  epoch_steps += 1
  print("epoch", epoch_steps, "training mce = %.6f" % train_accs[-1], "testing mce = %.6f" % test_mcs[-1])
  if train_accs[-1] <= best_train_error:
    break
